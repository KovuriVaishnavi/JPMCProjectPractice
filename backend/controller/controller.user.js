    ...
    try {
        ...
        if (users.length === 0) {
            ...
            if (!validPass) return res.status(401).json({ message: "Invalid login credentials! Please check it." });
                ...
                res.status(200).json({token:token,user:user.usertype,userDetails:user});
            } 
        }
    } catch (error) {
...
async function addFavouriteRecipe(req, res) {
    ...
    try {
      ...
      console.error("Error in addFavouriteRecipe:", error);
      return res.status(500).json({ message: "Internal server error" });
    }
}
...
async function addComment(req, res) {
    ...
    try {
        ...
        res.status(200).json({ message: "Comment added successfully", recipe: recipe });
    } catch (error) {
...
module.exports={signUpForm,loginForm,logout,getPreference,updatePreference,likeRecipe,addFavouriteRecipe,addComment,rateRecipe,removeComment,getuserdetails};
